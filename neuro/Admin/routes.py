...
from neuro.Admin.forms import LoginForm, RegistrationForm, WordForm
...
def user(user_id):
    ...
    return render_template('user.html',day=day,page_number=user_id,user=user,note=note)
...
def addAdmin():
    ...
    if form.validate_on_submit():
        ...
        user = User(name=form.name.data,phone=form.phone.data, email=form.email.data, password=hashed_password,confirmed=True, role='admin')
        db.session.add(user)
        db.session.commit()
        message="Pomyślne dodano admina"
    return render_template('add_admin.html',form=form,message=message)
...
def adminTest():
    day = Day.query.all()
    return render_template('admin_test.html',day=day)
...
def editTest(day_id):
    ...
    if request.method == 'POST':
        for words in word:
            ...
            db.session.commit()
    return render_template('admin_words.html',word=word,form=form,day_id=day_id)
